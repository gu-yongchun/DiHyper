...
class HypergraphConv(MessagePassing):
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor,
        ...
        if self.use_attention:
            ...
            x_i, x_j = x[hyperedge_index[0]], x[hyperedge_index[0]]
            alpha = (torch.cat([x_i, x_j], dim=-1) * self.att).sum(dim=-1)
            alpha = F.leaky_relu(alpha, self.negative_slope)
            alpha = softmax(alpha, hyperedge_index[0], num_nodes=x.size(0))
            alpha = F.dropout(alpha, p=self.dropout, training=self.training)

        if not self.symdegnorm: # HGNN^+ or HCHA wihtout attention 
...
class HCHA(nn.Module):
    ...
    def __init__(self, num_features, num_classes, args):
        ...
        self.use_attention = args.HCHA_att

#         Note that add dropout to attention is default in the original paper
        self.convs = nn.ModuleList()
        self.convs.append(HypergraphConv(num_features,
                                         args.MLP_hidden, self.symdegnorm, self.use_attention))
        for _ in range(self.num_layers-2):
            self.convs.append(HypergraphConv(
                args.MLP_hidden, args.MLP_hidden, self.symdegnorm, self.use_attention))
        # Output heads is set to 1 as default
        self.convs.append(HypergraphConv(
            args.MLP_hidden, num_classes, self.symdegnorm, self.use_attention))
